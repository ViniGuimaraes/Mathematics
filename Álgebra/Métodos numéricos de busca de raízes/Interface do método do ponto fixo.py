...
def guardar_parametros(tfEntradaF, tfEntradaG, tfx0, tfErro, tfPrecisao):
    ...
    if contar<200:
        ...
        lbRF = Label(janela, text = "Raiz = "+p.format(x0))
        lbIteracoes = Label(janela, text = "Número de iterações = "+str(iteracoes))
        lbErro = Label(janela, text = "Erro = "+p.format(abs(funcaoF(x0))))
        ...
        lbe = Label(janela, text = "A função g não converge para o ponto fixo")
        lbe.grid(row = 5, column = 1)
        
        
janela = Tk()

lbEntradaF = Label(janela, text = "Função f")
lbEntradaG = Label(janela, text = "Função g")
